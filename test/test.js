...
/* global $, QUnit, expect, module, test, asyncTest, start, ok, strictEqual, notStrictEqual */
...
$(function() {
  ...
  test('option', function() {
    expect($.support.xhrFormDataFileUpload ? 10 : 7);
    var eo = {
        originalEvent: {
          dataTransfer: { files: [{}], types: ['Files'] },
          clipboardData: { items: [{}] }
    ...
    $('#fileupload')
      .fileupload({
        send: function(e, data) {
          strictEqual(data.files[0].name, 'test', 'Triggers send callback');
        }
      })
  ...
  test('change', function() {
    ...
    fu.fileupload({
      change: function(e, data) {
        ok(true, 'Triggers change callback');
        strictEqual(data.files.length, 0, 'Returns empty files list');
      },
      add: $.noop
    });
    fuo._onChange({
  ...
  test('singleFileUploads', function() {
    ...
    $('#fileupload')
      ...
      .fileupload('add', param)
      .fileupload('option', 'singleFileUploads', false)
      .fileupload('add', param);
  });
  ...
  test('destroy', function() {
    ...
    $('#fileupload').fileupload({
      destroy: function(e, data) {
        ...
        strictEqual(data.url, 'test', 'Passes over deletion url parameter');
        strictEqual(
          data.type,
          'DELETE',
  ...
  test('maxNumberOfFiles', function() {
    ...
    $('#fileupload')
      .fileupload({
        ...
        singleFileUploads: false,
        send: function() {
          strictEqual((sendIndex += 1), addIndex);
        },
        progress: $.noop,
        progressall: $.noop,
        done: $.noop,
  ...
  test('maxFileSize', function() {
    ...
    $('#fileupload')
      ...
      .fileupload({
        send: function(e, data) {
          ok(!$.blueimp.fileupload.prototype.options.send.call(this, e, data));
          return false;
        }
      });
  });
  ...
  test('acceptFileTypes as HTML5 data attribute', function() {
    ...
    strictEqual(
      $('#fileupload')
        .fileupload('option', 'acceptFileTypes')
        .toString(),
      regExp.toString()
    );
  });
